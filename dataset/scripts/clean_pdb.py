...
lines, filename_stem, dir_ = open_pdb( args[0] )
...
if nres > 0:
    if not options.nopdbout:
        ...
        outfile = os.path.join(dir_, filename_stem + "_" + chainid + ".pdb")
    ...
    if not options.allchains:
        for chain in fastaseq:
            ...
            fastaid.write('\n')
            handle = open(os.path.join(dir_, filename_stem+"_"+"".join(chain) + ".fasta"), 'w')
        ...
        fastaid.write('\n')
        handle = open(os.path.join(dir_, filename_stem+"_"+chainid + ".fasta"), 'w')
