...
def plot4metrics(x_labels, v1, v2, v3, v_labels, range_, top_n, y_label_prefix):
    ...
    ax2 = ax.twinx()
    ax2.set_yticks([])
    ax2.set_ylabel("RoseTTAFold", weight='bold', rotation="90")
    ...
    fig.tight_layout()
    path = "../output/" + y_label_prefix + "_" + range_ + "_" + top_n[:4] + top_n[-1] + "_RoseTTAFold"
...
def get_contacts(dir, pro):
    npz = os.path.join(dir, pro, "t000_.e2e.npz")
    if not os.path.exists(npz): return "empty"
    dist = np.load(npz)['dist']
    contact_mtx = np.sum(dist[:, :, :CONTACT_BINCUTOFF], axis=-1) # 2-20A, 36 bins
    ...
    if args.plot:
        ...
        for range_ in ranges:
            for top_n in top_describe:
                ...
                # add empty data for 6XMS and 6XMU
                pro_labels = ['6XMS_A', '6XMU_A'] + (pro_labels) # as the method failed for the target protein
                print(p_cos)
                p_cos = [0, 0] + (p_cos)
                print(p_cos)
                p1s = [0, 0] + (p1s)
