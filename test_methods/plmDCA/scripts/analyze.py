...
def plot4metrics(x_labels, v1, v2, v3, v_labels, range_, top_n, y_label_prefix):
    ...
    ax2 = ax.twinx()
    ax2.set_yticks([])
    ax2.set_ylabel("PSICOV", weight='bold', rotation="90")
    ...
    fig.tight_layout()
    path = "../output/" + y_label_prefix + "_" + range_ + "_" + top_n[:4] + top_n[-1] + "_PSICOV"
    ...
    plt.clf()
    plt.close()
    print("../output/" + y_label_prefix + "_" + range_ + "_" + top_n[:4] + top_n[-1] + "_PSICOV")
...
def get_contacts(dir_, pro):
    ...
    if os.stat(c_file).st_size == 0: return 'empty'
    with open(c_file, 'r') as f:
        for line in f:
            lst = line.split(',')
            i, j, p = lst[:3]
            #p = lst[4]
            contact_mtx[int(i)-1,int(j)-1] = p
            contact_mtx[int(j)-1,int(i)-1] = p
    ...
    if args.analyze:
        ...
        for pair in dic_distance:
            ...
            for p in range(len(pros)):
                ...
                raw_fasta_file = os.path.join(args.dir,
                        pro, pro+".fasta")
                seq2 = list(SeqIO.parse(raw_fasta_file, "fasta"))[0].seq
                print("Checking ", pro)
