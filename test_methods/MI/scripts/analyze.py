...
def plot4metrics(x_labels, v1, v2, v3, v_labels, range_, top_n, y_label_prefix):
    ...
    ax2 = ax.twinx()
    ax2.set_yticks([])
    ax2.set_ylabel("MI", weight='bold', rotation="90")
    colors = ['C1', 'C2'] * int(len(x_labels)/2)
    for xtick, color in zip(ax.get_xticklabels(), colors):
            xtick.set_color(color)

    fig.tight_layout()
    path = "../output/" + y_label_prefix + "_" + range_ + "_" + top_n[:4] + top_n[-1]
...
def get_contacts(dir_, pro):
    ...
    with open(c_file, 'r') as f:
        for line in f:
            ...
            contact_mtx[int(i)-1,int(j)-1] = mi
            contact_mtx[int(j)-1,int(i)-1] = mi
    ...
    if args.analyze:
        ...
        for pair in dic_distance:
            ...
            for p in range(len(pros)):
                ...
                raw_fasta_file = os.path.join("../output/dataset",
                        pro, pro+".fasta")
                seq2 = list(SeqIO.parse(raw_fasta_file, "fasta"))[0].seq
                print("Checking ", pro)
    ...
    if args.plot:
        ...
        for range_ in ranges:
            for top_n in top_describe:
                ...
                plot4metrics(pro_labels, p_cos, p1s, p2s, ["shared","IF","OF"], range_, top_n, y_label_prefix="Precision")
                plot4metrics(pro_labels, c_cos, c1s, c2s, ["shared","IF","OF"], range_, top_n, y_label_prefix="Coverage")
