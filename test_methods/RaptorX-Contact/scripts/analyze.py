...
def plot4metrics(x_labels, v1, v2, v3, v_labels, range_, top_n, y_label_prefix):
    ...
    rect1 = ax.bar(x- width*3.0/2, v1, width, color="C0")
    rect2 = ax.bar(x- width/2.0, v2, width, color="C1")
    rect3 = ax.bar(x+ width/2.0, v3, width, color="C2")
        ...
        *xpos* indicates which side to place the text w.r.t. the center of
        ...
        for rect in rects:
            ...
            ax.text(rect.get_x() + rect.get_width()*offset[xpos], 1.01*height,
                    '{:.4f}'.format(height), ha=ha[xpos], va='bottom', rotation='vertical',size=2)
    ...
    ax2 = ax.twinx()
    ax2.set_yticks([])
    ax2.set_ylabel("RaptorX-C", weight='bold', rotation="90")
    ...
    fig.tight_layout()
    path = "../output/" + y_label_prefix + "_" + range_ + "_" + top_n[:4] + top_n[-1] + "_raptorx"
...
def plot_acc(data):
    ...
    rects1 = ax.bar(x - width/2.0, acc1, width, label="Acc. in state1", color='green')
    rects2 = ax.bar(x + width/2.0, acc2, width, label="Acc. in state2", color='red')
    ...
    ax2 = ax.twinx()
    rects_co = ax2.bar(x+3*width/2.0, acc_co, width, label="Acc. in common",color='blue')
...
def get_contacts(dir_, pro):
    pkl = os.path.join(dir_, pro, pro+".predictedDistMatrix.pkl")
    if not os.path.exists(pkl): return 'empty'
    f = open(pkl, 'rb')
    data = pickle.load(f, encoding='latin1')
    f.close()
    contact_mtx = data[3]['CbCb']
