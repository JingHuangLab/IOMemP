...
def plot4metrics(x_labels, v1, v2, v3, v_labels, range_, top_n, y_label_prefix):
    ...
    ax2 = ax.twinx()
    ax2.set_yticks([])
    ax2.set_ylabel("CCMpred", weight='bold', rotation="90")
    ...
    fig.tight_layout()
    path = "../output/" + y_label_prefix + "_" + range_ + "_" + top_n[:4] + top_n[-1] + "_CCMpred"
    ...
    plt.clf()
    plt.close()
    print("../output/" + y_label_prefix + "_" + range_ + "_" + top_n[:4] + top_n[-1] + "_CCMpred")
...
def get_contacts(dir_, pro):
    ...
    c_file = os.path.join(dir_, pro, pro+".ccmpred")
    contact_mtx = np.genfromtxt(c_file)

    assert contact_mtx.shape[0] == length
