...
def plot4metrics(x_labels, v1, v2, v3, v_labels, range_, top_n, y_label_prefix):
    ...
    ax2 = ax.twinx()
    ax2.set_yticks([])
    ax2.set_ylabel("trRosetta", weight='bold', rotation="90")
    ...
    fig.tight_layout()
    path = "../output/" + y_label_prefix + "_" + range_ + "_" + top_n[:4] + top_n[-1] + "_trRosetta"
...
def get_contacts(dir_, pro):
    npz = os.path.join(dir_, pro, pro+".npz")
    dist = np.load(npz)['dist']
    contact_mtx =  np.sum(dist[:, :, 1:13], axis=2) # 2-20A, 36 bins
    ...
    if args.plot:
        ...
        for range_ in ranges:
            for top_n in top_describe:
                ...
                plot4metrics(pro_labels, c_cos, c1s, c2s, ["Shared","IF","IF"], range_, top_n, y_label_prefix="Coverage")
